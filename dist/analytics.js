...
export async function trackVisitor(ip, path, referrer, userAgent) {
    const sql = getSql();
    try {
...
export async function getVisitorAnalytics(timeframe = '24h') {
    const sql = getSql();
    const interval = {
        '24h': '1 day',
        '7d': '7 days',
	...
	`;
    return {
        totalVisitors: parseInt(String(visitorStats[0]?.total_visitors ?? '0')),
        uniqueVisitors: parseInt(String(visitorStats[0]?.unique_visitors ?? '0')),
        pageViews: pageViews,
